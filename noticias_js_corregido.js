...
function aplicarOrdenamiento() {
    ...
    if (orden === 'aleatorio') {
        // Para ordenamiento aleatorio, primero ordenar por fecha (más recientes primero)
        // y luego mezclar solo las noticias más recientes (últimos 3 días)
        const tresDiasAtras = new Date();
        tresDiasAtras.setDate(tresDiasAtras.getDate() - 3);
        
        // Separar noticias recientes de antiguas
        const noticiasRecientes = [];
        const noticiasAntiguas = [];
        
        noticiasFiltradas.forEach(noticia => {
            const fechaNoticia = new Date(noticia.fecha_publicacion);
            if (fechaNoticia >= tresDiasAtras) {
                noticiasRecientes.push(noticia);
            } else {
                noticiasAntiguas.push(noticia);
            }
        });
        
        // Mezclar aleatoriamente solo las noticias recientes
        noticiasRecientes.sort(() => Math.random() - 0.5);
        
        // Mantener las noticias antiguas ordenadas por fecha (más recientes primero)
        noticiasAntiguas.sort((a, b) => new Date(b.fecha_publicacion) - new Date(a.fecha_publicacion));
        
        // Combinar: recientes mezcladas + antiguas ordenadas
        noticiasFiltradas = [...noticiasRecientes, ...noticiasAntiguas];
    } else {
        // Para otros ordenamientos, usar la lógica normal
