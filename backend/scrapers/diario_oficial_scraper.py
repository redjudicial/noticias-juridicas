...
Extrae noticias completas de https://www.diariooficial.interior.gob.cl
"""
...
class DiarioOficialScraper:
    ...
    def __init__(self, openai_api_key: str = None):
        self.base_url = "https://www.diariooficial.interior.gob.cl"
        self.noticias_url = "https://www.diariooficial.interior.gob.cl/publicaciones"
    ...
    def get_noticias_recientes(self, max_noticias: int = 20) -> List[Dict]:
        ...
        try:
            print(f"🔍 Obteniendo noticias del Diario Oficial...")
            ...
            noticias_container = soup.find('div', class_='publicaciones') or soup.find('main') or soup.find('body')
    ...
    def get_noticia_completa(self, url: str, titulo: str = None) -> Optional[NoticiaCompleta]:
        ...
        try:
            ...
            # Extraer información específica del Diario Oficial
            noticia = self._extract_noticia_diario_oficial(soup, url, titulo)
    ...
    def _extract_noticia_diario_oficial(self, soup: BeautifulSoup, url: str, titulo: str = None) -> Optional[NoticiaCompleta]:
        """Extraer noticia específica del Diario Oficial"""
        try:
            # Extraer título
            titulo_elem = soup.find('h1') or soup.find('h2') or soup.find('title')
            titulo_final = titulo or (titulo_elem.get_text(strip=True) if titulo_elem else "Publicación Diario Oficial")
            
            # Extraer fecha
            fecha = self._extract_fecha_diario_oficial(soup)
            
            # Extraer contenido
            contenido = self._extract_contenido_diario_oficial(soup)
            ...
            noticia = NoticiaCompleta(
                ...
                fuente_nombre_completo='Diario Oficial de Chile',
                url_origen=url,
                categoria='oficial',
                tipo_documento='publicacion_oficial',
            ...
            print(f"❌ Error extrayendo noticia del Diario Oficial: {e}")
            return None
    
    def _extract_fecha_diario_oficial(self, soup: BeautifulSoup) -> datetime:
        """Extraer fecha de noticia del Diario Oficial"""
        try:
            ...
            for selector in fecha_selectors:
                ...
                if fecha_elem:
                    ...
                    fecha_parsed = self._parse_fecha_diario_oficial(fecha_texto)
                    if fecha_parsed:
                        return fecha_parsed
    ...
    def _parse_fecha_diario_oficial(self, fecha_texto: str) -> Optional[datetime]:
        """Parsear fecha del Diario Oficial"""
        try:
            # Patrones de fecha comunes en el Diario Oficial
            patrones = [
                r'(\d{1,2})/(\d{1,2})/(\d{4})',
                r'(\d{1,2})-(\d{1,2})-(\d{4})',
                r'(\d{4})-(\d{1,2})-(\d{1,2})',
    ...
    def _extract_contenido_diario_oficial(self, soup: BeautifulSoup) -> str:
        """Extraer contenido de noticia del Diario Oficial"""
        try:
            # Buscar contenido en diferentes selectors
            contenido_selectors = [
                '.contenido', '.content', '.publicacion-contenido',
                '.post-content', '.entry-content', 'article',
                '.publicacion-texto', '.texto-publicacion'
            ]
    ...
    def _is_noticia_link(self, href: str) -> bool:
        ...
        try:
            # Patrones de URLs de noticias del Diario Oficial
            patrones = [
                r'/publicaciones/',
                r'/noticias/',
                r'/comunicados/',
    ...
    def _extract_fecha_link(self, link_elem) -> Optional[datetime]:
        ...
        try:
            ...
            return self._parse_fecha_diario_oficial(fecha_texto)
        except Exception:
            return None
    ...
    def scrape_noticias_recientes(self, max_noticias: int = 10) -> List[NoticiaCompleta]:
        ...
        try:
            print(f"🚀 Iniciando scraping del Diario Oficial...")
