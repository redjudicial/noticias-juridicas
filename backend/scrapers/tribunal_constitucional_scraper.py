#!/usr/bin/env python3
"""
Scraper para el Tribunal Constitucional de Chile
Extrae noticias completas de https://www.tribunalconstitucional.cl
"""
...
class TribunalConstitucionalScraper:
    ...
    def __init__(self, openai_api_key: str = None):
        self.base_url = "https://www.tribunalconstitucional.cl"
        self.noticias_url = "https://www.tribunalconstitucional.cl/prensa/noticias/"
    ...
    def get_noticias_recientes(self, max_noticias: int = 20) -> List[Dict]:
        ...
        try:
            print(f"🔍 Obteniendo noticias del Tribunal Constitucional...")
    ...
    def get_noticia_completa(self, url: str, titulo: str = None) -> Optional[NoticiaCompleta]:
        ...
        try:
            ...
            # Extraer información específica del Tribunal Constitucional
            noticia = self._extract_noticia_tribunal_constitucional(soup, url, titulo)
    ...
    def _extract_noticia_tribunal_constitucional(self, soup: BeautifulSoup, url: str, titulo: str = None) -> Optional[NoticiaCompleta]:
        """Extraer noticia específica del Tribunal Constitucional"""
        try:
            # Extraer título
            titulo_elem = soup.find('h1') or soup.find('h2') or soup.find('title')
            titulo_final = titulo or (titulo_elem.get_text(strip=True) if titulo_elem else "Noticia Tribunal Constitucional")
            
            # Extraer fecha
            fecha = self._extract_fecha_tribunal_constitucional(soup)
            
            # Extraer contenido
            contenido = self._extract_contenido_tribunal_constitucional(soup)
            ...
            print(f"❌ Error extrayendo noticia del Tribunal Constitucional: {e}")
            return None
    
    def _extract_fecha_tribunal_constitucional(self, soup: BeautifulSoup) -> datetime:
        """Extraer fecha de noticia del Tribunal Constitucional"""
        try:
            ...
            for selector in fecha_selectors:
                ...
                if fecha_elem:
                    ...
                    fecha_parsed = self._parse_fecha_tribunal_constitucional(fecha_texto)
                    if fecha_parsed:
                        return fecha_parsed
    ...
    def _parse_fecha_tribunal_constitucional(self, fecha_texto: str) -> Optional[datetime]:
        """Parsear fecha del Tribunal Constitucional"""
        try:
            # Patrones de fecha comunes en el Tribunal Constitucional
            patrones = [
                r'(\d{1,2})/(\d{1,2})/(\d{4})',
                r'(\d{1,2})-(\d{1,2})-(\d{4})',
                r'(\d{4})-(\d{1,2})-(\d{1,2})',
    ...
    def _extract_contenido_tribunal_constitucional(self, soup: BeautifulSoup) -> str:
        """Extraer contenido de noticia del Tribunal Constitucional"""
        try:
    ...
    def _is_noticia_link(self, href: str) -> bool:
        ...
        try:
            # Patrones de URLs de noticias del Tribunal Constitucional
            patrones = [
                r'/noticias/',
                r'/prensa/',
                r'/comunicados/',
    ...
    def _extract_fecha_link(self, link_elem) -> Optional[datetime]:
        ...
        try:
            ...
            return self._parse_fecha_tribunal_constitucional(fecha_texto)
        except Exception:
            return None
    ...
    def scrape_noticias_recientes(self, max_noticias: int = 10) -> List[NoticiaCompleta]:
        ...
        try:
            print(f"🚀 Iniciando scraping del Tribunal Constitucional...")
