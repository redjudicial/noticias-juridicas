#!/usr/bin/env python3
"""
Scraper para el Primer Tribunal Ambiental de Chile
"""
import sys
import os
...
class PrimerTribunalAmbientalScraper(BaseScraper):
    """Scraper para el Primer Tribunal Ambiental de Chile"""
    
    def __init__(self, openai_api_key: str = None):
        super().__init__(openai_api_key)
        self.base_url = "https://www.1ta.cl"
        self.noticias_url = "https://www.1ta.cl/category/noticias/"
    ...
    def get_noticias_recientes(self, max_noticias: int = 20) -> List[Dict]:
        """Extrae las noticias más recientes del Primer Tribunal Ambiental"""
        try:
            print(f"🔍 Extrayendo noticias de 1TA...")
            ...
            print(f"✅ 1TA: {len(noticias)} noticias encontradas")
            return noticias
    ...
    def procesar_noticia(self, noticia_raw: Dict) -> Optional[NoticiaEstandarizada]:
        ...
        try:
            ...
            titulo_con_prefijo = f"(1º) {titulo_original}"
            
            # Aplicar prefijo al contenido si no empieza con el prefijo
            if not contenido.startswith("(1º)"):
                contenido_con_prefijo = f"(1º) {contenido}"
            else:
            ...
            print(f"❌ Error procesando noticia 1TA: {str(e)}")
            return None

    def normalizar_fecha(self, fecha_str: str) -> datetime:
        """Normaliza diferentes formatos de fecha del 1TA"""
        try:
            if not fecha_str:
